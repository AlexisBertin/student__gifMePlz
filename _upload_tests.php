...
if(!isset($_SESSION['token'])){
	...
	if($_SESSION['photoCount'] == 8){
		$sql = "SELECT id FROM users WHERE randNumber = '".$_SESSION['token']."'";
		try {
		   $req = $connexion->prepare($sql);
		   $idCurrentUser = $req->execute();


		   	$sql = $connexion->prepare("INSERT INTO photos (idUser, neutre, haine, joie, degout, tristesse, peur, surprise, douleur, gif) VALUES ('".$idCurrentUser."','".$_SESSION['shootingSession'][0]."','".$_SESSION['shootingSession'][1]."','".$_SESSION['shootingSession'][2]."','".$_SESSION['shootingSession'][3]."','".$_SESSION['shootingSession'][4]."','".$_SESSION['shootingSession'][5]."','".$_SESSION['shootingSession'][6]."','".$_SESSION['shootingSession'][7]."',''");
		   	try {
		       	$connexion->execute($sql, $_SESSION['shootingSession']);
		       } catch(PDOException $e) {
		   		echo 'erreur: '.$e->getMessage();
		       }
